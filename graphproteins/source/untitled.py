...
dist_matrix = np.genfromtxt('distance_matrix_zeroligand.csv', delimiter=',', skip_header=1)
header = np.genfromtxt('distance_matrix_zeroligand.csv', delimiter=',', dtype=str, max_rows=1)
mean = np.mean(dist_matrix)
std = np.std(dist_matrix)
dist_matrix = (dist_matrix ) / std
dist_mask = np.where(dist_matrix > 1.5, 0, dist_matrix)
...
spring_3D = nx.spring_layout(G_relab,dim=3, seed=18)
#print(spring_3D)
...
layout = go.Layout(title="Network, Cutoff = 1.5",
                width=1000,
                height=1000,
                showlegend=False,
